...
public class AddArticleController implements Initializable {
    ...
    public void emojiPopup() throws IOException {
        Parent root = FXMLLoader.load(getClass().getResource("/tn/esprit/pidev/FXML/emojis.fxml"));
    ...
    void handleSave(ActionEvent event) {
        ...
        // Get the current user's ID from the static User.connecte field
        if (tn.esprit.pidev.Model.User.connecte != null) {
            article.setUserId(tn.esprit.pidev.Model.User.connecte.getId());
        } else {
            // If no user is logged in, show an error
            errorLabel.setText("Vous devez être connecté pour ajouter un article !");
            return;
        }
